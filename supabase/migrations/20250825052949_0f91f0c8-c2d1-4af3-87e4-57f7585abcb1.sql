...
BEGIN
  ...
  -- Check for suspicious rapid access patterns (potential automated attacks)
  ...
  IF request_type = 'private_key_access' THEN
    -- Additional validation for private key access
    IF NOT EXISTS (
      SELECT 1 FROM wallet_auth_tokens 
      WHERE user_id = current_user_id 
      AND is_active = true
...
-- Enhanced function to detect advanced wallet threats
CREATE OR REPLACE FUNCTION public.detect_advanced_wallet_threats()
RETURNS TABLE(
  user_id text, 
  threat_type text, 
  threat_level text, 
...
BEGIN
  RETURN QUERY
  -- Detect automated wallet harvesting attempts
  SELECT 
    sl.identifier,
    'automated_wallet_harvesting' as threat_type,
    'critical' as threat_level,
    COUNT(*) as event_count,
    MAX(sl.created_at) as latest_incident,
    'Immediately suspend account and investigate for bot activity' as recommendation
  FROM security_logs sl
  WHERE sl.event_type LIKE '%wallet%'
  ...
  -- Detect cross-wallet access attempts (potential account takeover)
  SELECT 
    sl.identifier,
    'cross_wallet_exploitation' as threat_type,
    'critical' as threat_level,
    COUNT(*) as event_count,
    MAX(sl.created_at) as latest_incident,
    'Emergency wallet freeze and security investigation required' as recommendation
  FROM security_logs sl
  WHERE sl.event_type IN ('wallet_unauthorized_access_ultra', 'wallet_token_unauthorized_access')
  ...
  -- Detect private key access attempts
  SELECT 
    sl.identifier,
    'private_key_extraction_attempt' as threat_type,
    'critical' as threat_level,
    COUNT(*) as event_count,
    MAX(sl.created_at) as latest_incident,
    'Immediate wallet isolation and forensic analysis required' as recommendation
  FROM security_logs sl
  WHERE sl.event_type = 'private_key_access_blocked'
...
WITH CHECK (false); -- Service role cannot create new wallets
...
CREATE POLICY "Block all user wallet token modifications"
ON public.wallet_auth_tokens
...
GRANT EXECUTE ON FUNCTION public.detect_advanced_wallet_threats TO authenticated;