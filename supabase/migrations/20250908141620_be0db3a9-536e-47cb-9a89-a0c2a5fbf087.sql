-- Enhanced Auth Token Security Fix
...
-- Create secure function to log auth token access attempts
...
AS $$
BEGIN
  -- Log all auth token access attempts for security monitoring
  IF TG_OP = 'SELECT' THEN
    INSERT INTO security_logs (event_type, identifier, details, severity)
    VALUES (
      'auth_token_data_accessed',
      COALESCE(NEW.email, OLD.email),
      jsonb_build_object(
        'user_id', auth.uid(),
        'token_id', COALESCE(NEW.id, OLD.id),
        'access_type', 'select_operation',
        'auth_role', auth.role(),
        'timestamp', NOW(),
        'user_agent', current_setting('request.headers', true)::json->>'user-agent'
      ),
      CASE 
        WHEN auth.role() = 'service_role' THEN 'low'
        ELSE 'medium'
      END
    );
