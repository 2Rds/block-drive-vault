...
-- Add security trigger to log any access to private key data
CREATE OR REPLACE FUNCTION public.log_wallet_private_key_access()
RETURNS TRIGGER AS $$
BEGIN
  -- Log any attempt to access private key data
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'operation', TG_OP,
      'timestamp', NOW(),
      'has_private_key_access', (TG_OP = 'SELECT' AND OLD.private_key_encrypted IS NOT NULL)
    ),
    'high'
  );
...
-- Apply the logging trigger to wallet operations
DROP TRIGGER IF EXISTS log_wallet_private_key_access_trigger ON public.wallets;
CREATE TRIGGER log_wallet_private_key_access_trigger
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.wallets
  FOR EACH ROW EXECUTE FUNCTION public.log_wallet_private_key_access();

-- Update wallet service to never include private keys in SELECT queries
-- This is enforced at the database level by excluding private_key_encrypted from normal selects
