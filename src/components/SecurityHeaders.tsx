...
export const SecurityHeaders = () => {
  useEffect(() => {
    ...
    if (!document.querySelector('meta[http-equiv="Content-Security-Policy"]')) {
      ...
      meta.content = `
        default-src 'self';
        script-src 'self' 'sha256-' https://*.supabase.co https://*.dynamic.xyz https://widget.intercom.io;
        ...
        connect-src 'self' https://*.supabase.co wss://*.supabase.co https://*.dynamic.xyz https://api.intercom.io;
        frame-src 'self' https://*.dynamic.xyz https://widget.intercom.io;
        object-src 'none';
        base-uri 'self';
        form-action 'self';
