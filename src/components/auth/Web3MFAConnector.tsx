...
export const Web3MFAConnector = ({
  ...
  const handleStartAuthentication = async () => {
    ...
    try {
      ...
      const authResult = await Web3AuthService.authenticateUser(walletAddress, blockchainType, signature, message);
  ...
  const handleSubdomainRegistration = async () => {
    ...
    try {
      const walletAddress = primaryWallet.address;
      const blockchainType = getBlockchainType(primaryWallet);
      const result = await Web3AuthService.registerSubdomain(walletAddress, blockchainType, subdomainName.trim());
      if (result.success) {
    ...
    switch (authStep) {
      case 'connect':
        return <div className="space-y-4">
            ...
            {primaryWallet ? <div className="space-y-4">
                ...
                <Button onClick={handleStartAuthentication} disabled={loading} className="w-full bg-blue-600 hover:bg-blue-700">
                  {loading ? <>
            ...
            <div className="space-y-3">
              <div>
                ...
                <div className="flex items-center space-x-2">
                  <Input placeholder="yourname" value={subdomainName} onChange={e => setSubdomainName(e.target.value.toLowerCase().replace(/[^a-z0-9]/g, ''))} className="flex-1" />
                  <span className="text-gray-400 text-sm">
                    .blockdrive.{domainSuffix}
                  </span>
                </div>
                {subdomainName && <p className="text-sm text-gray-400 mt-1">
                    Your subdomain: <span className="text-white">{subdomainName}.blockdrive.{domainSuffix}</span>
                  </p>}
              </div>
              
              <Button onClick={handleSubdomainRegistration} disabled={loading || !subdomainName.trim()} className="w-full bg-purple-600 hover:bg-purple-700">
                {loading ? <>
                    <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                    Registering...
                  </> : <>
  ...
  return <Card className="bg-gray-900/40 border-gray-800">
